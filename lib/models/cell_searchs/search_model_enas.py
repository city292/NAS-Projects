...
class TinyNetworkENAS(nn.Module):
    ...
    def __init__(self, C, N, max_nodes, num_classes, search_space):
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(3, C, kernel_size=3, padding=5, bias=False),
            nn.BatchNorm2d(C))

        layer_channels   = [C    ] * N + [C*2 ] + [C*2  ] * N + [C*4 ] + [C*4  ] * N + [C*8 ] + [C*8  ] * N + [C*8 ] + [C*8  ] * N + [C*8 ] + [C*8  ] * N
        layer_reductions = [False] * N + [True] + [False] * N + [True] + [False] * N + [True] + [False] * N + [True] + [False] * N + [True] + [False] * N

        C_prev, num_edge, edge2index = C, None, None
        self.cells = nn.ModuleList()
        for index, (C_curr, reduction) in enumerate(zip(layer_channels, layer_reductions)):
            if reduction:
                cell = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
